...
class HomeView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ).select_related('destination').order_by('-created_at')[:6]
        context['latest_gallery_images'] = GalleryImage.objects.all().order_by('-uploaded_at')[:8]
        return context


def home(request):
    """Function-based view alternative for homepage"""
    context = {
        ...
        'latest_gallery_images': GalleryImage.objects.all().order_by('-uploaded_at')[:8],
    }
